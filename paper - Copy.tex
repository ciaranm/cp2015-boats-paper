...
Firstly, we note that we have rephrased criteria 2 \& 3 (denoted *) to clarify an error in \cite{isaf}. The order of matches within a flight is significant and is constrained, as is the order of flights. This permits fair schedules that provide the opportunity for skippers to change boats, etc. Criteria 12 defines what is meant by a boat change: if in flight $i$ this skipper is not in a match (i.e. it is a bye for this skipper) but is in a match in flight $i+1$ then he has to get into a boat, rearrange all his kit and set the boat as he likes it, and this is a boat change.
...
Criteria 10 is ambigious: - this could be interpreted on an schedule basis: i.e.\ to minimise the overall number of changes in the entire pairing list schedule; or alterantively as well as a fair, but minimal number of changes across all skippers. In our work, we take the latter viewpoint.
