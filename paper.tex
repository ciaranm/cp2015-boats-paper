...
of his schedule in the array of constrained integer variables $timeSlot_{0}$
to $timeSlot_{t-1}$ (equation \ref{eq3} below), and a state view in the array of
constraint integer variables $state_{0}$ to $state_{f-1}$ (equation \ref{eq4}
below), i.e. $state_{i}$ is the state of this skipper in flight $i$ and
corresponds to timeslots $timeSlot_{i*m}$ to $timeSlot_{m.(i+1)-1}$.
