...
\authorrunning{McCreesh, MacDonald, Miller and Prosser}
\tocauthor{Patrick Prosser}
\institute{School of Computing Science, University of Glasgow, fname.sname@glasgow.ac.uk}
\maketitle

\begin{abstract} 
The quick brown duck jumped over the lazy dog. The quick brown duck jumped over the lazy dog. The quick brown duck jumped over the lazy dog. The quick brown duck jumped over the lazy dog.
\end{abstract}
...
& \forall_{i \in [0..f]} ~ state_{i} = FIRST \iff timeSlot_{i.m} \geq 0  \label{eq5} \\
& \forall_{i \in [0..f]} ~ state_{i} = MID \iff  \exists_{j \in [i.m+1 .. i.m+m-2]} ~ timeSlot_{j} \geq 0  \label{eq6} \\
& \forall_{i \in [0..f]} ~ state_{i} = LAST \iff timeSlot_{(i+1).m - 1} \geq 0  \label{eq7} \\
...
& \forall_{i \in [0..n-2]} ~ \forall_{j \in [i+1..n-1]}  ~ modMatch_{i,j} = \frac{match_{i,j}}{m} \label{eq18} \\
...
& \forall_{i \in [0..m-1]} ~ \forall_{j \in [0..f-1]} ~ \sigma.position_{i,j} = 1 \iff \sigma.timeSlot_{m.j+i} \geq 0 \label{posB} \\
& \forall_{i \in [0..m-1]} ~ imbalance_{i} = |\frac{n-1}{m} - \sum_{j=0}^{f-1} position_{i,j}| \label{imbA} \\
