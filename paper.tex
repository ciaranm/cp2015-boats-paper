...
of his schedule in the array of constrained integer variables $\timeSlot[0]$ to
$\timeSlot[{t-1}]$ (\eqref{var:timeSlot} below), and a state view in the array
of constraint integer variables $\state[0]$ to $\state[{f-1}]$
(\eqref{var:state} below), i.e.\ $\state[i]$ is the state of this skipper in
flight $i$ and corresponds to timeslots $\timeSlot[{i \cdot m}]$ to
$\timeSlot[{m(i+1)-1}]$.
...
\begin{framed}
\begin{align}
    &f = \lceil n(n-1)/m  \rceil \label{var:f} \tag{V1} \\
    &t = n(n-1)/2 \label{var:t} \tag{V2} \\
    &\forall \tau \in \{0 \nldots t-1\}: \timeSlot[{\tau}] \in \{-2 \nldots t-1\} \setminus \{\sigma\} \label{var:timeSlot} \tag{V3} \\
    &\forall i \in \{0 \nldots f-1\}: \state[{i}] \in \{\FIRST,\MID,\LAST,\BYE,\END\} \label{var:state} \tag{V4} \\
